...
function countVowels(str) {
  ...
  let vowelCount = 0;
  for (let i = 0; i < str.length; i += 1) {
    const currentChar = str.charAt(i).toLowerCase();
...
function isPalindrome(str) {
  const clearedStr = str.toLowerCase().split(' ').join('').replace(/\W/g, '');
...
function reverseWords(str) {
  ...
  reversedArr.forEach((word) => {
    newArr.push(word.split('').reverse().join('').replace(/\W/g, ''));
  });
  return newArr.join(' ');
}
...
function invertCase(str) {
  let answer = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].search(/\w/g) !== -1 && str[i].toLowerCase() === str[i]) {
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (
      alphabet.indexOf(str.charAt(i)) !== -1 ||
      alphabet.indexOf(str.charAt(i).toLowerCase()) !== -1
    ) {
      const currentChar = str.charAt(i).toLowerCase();
      const isLowerCase = alphabet.indexOf(str.charAt(i)) !== -1;
      const indexCharInAlphabet = alphabet.indexOf(currentChar);
      let newChar = '';
      if (indexCharInAlphabet + 13 > alphabet.length - 1) {
        newChar += alphabet.charAt(indexCharInAlphabet + 13 - 26);
      } else {
        newChar += alphabet.charAt(indexCharInAlphabet + 13);
      }
      newStr += isLowerCase ? newChar : newChar.toUpperCase();
    } else {
      newStr += str.charAt(i);
    }
  }
